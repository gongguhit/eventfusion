...
private:
	...
	std::atomic<bool> waitForReset{true};
	...
	struct SortedPacketBuffers {
		...
		[[nodiscard]] std::optional<dv::cvector<dv::Trigger>> popTriggers(const int64_t timeOffset) {
			...
			return convertTriggerPacket(packet.second, timeOffset, triggerStreamSeek);
		}
	};
	...
	[[nodiscard]] static inline dv::Frame convertFramePacket(
		...
		return {timestampOffset + caerFrameEventGetTSStartOfExposure64(event, frame), exposure,
	...
	[[nodiscard]] static dv::EventStore convertEventsPacket(
		const std::shared_ptr<libcaer::events::EventPacket> &packet, const int64_t timestampOffset) {
		auto eventPtr           = std::make_shared<dv::EventPacket>();
		...
		eventPtr->elements.reserve(static_cast<size_t>(numEvents));
		for (int32_t i = 0; i < numEvents; i++) {
			caerPolarityEvent event = caerPolarityEventPacketGetEvent(eventHeader, i);
			eventPtr->elements.emplace_back(timestampOffset + caerPolarityEventGetTimestamp64(event, eventHeader),
				caerPolarityEventGetX(event), caerPolarityEventGetY(event), caerPolarityEventGetPolarity(event));
		}
		return dv::EventStore(eventPtr);
	}
	...
	void sendTimestampReset() {
		...
		switch (discoveryResult.deviceType) {
			case CAER_DEVICE_DVXPLORER:
				// Mini does not currently support TS reset.
				...
				if (self->keepRunning.load(std::memory_order_relaxed)) {
					...
					if (auto packetContainer = self->device->dataGet(); packetContainer != nullptr) {
						for (auto &packet : *packetContainer) {
							...
							// DVXplorer Mini: no TS reset supported, so we just take the first data to arrive if we're
							// waiting on reset.
							if (containsResetEvent(packet)
								|| (self->isDeviceDVXplorerMini()
									&& self->waitForReset.load(std::memory_order_relaxed))) {
								self->setTimestampOffset(dv::now());
								self->waitForReset = false;

								// Skip this whole packet container if a reset is found.
