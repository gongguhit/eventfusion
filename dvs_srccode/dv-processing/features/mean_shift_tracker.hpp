...
private:
	...
	[[nodiscard]] Result::SharedPtr updateTracks(const cv::Mat &normalizedTimeSurface) {
		...
		for (const auto &inputTrack : lastFrameResults->keypoints) {
			...
			const auto isWithinRangeOfOldTracks = [&newCandidateCenter, this](const auto newStoredCenters) {
			...
			const bool isInRange = (std::none_of(newCenters.begin(), newCenters.end(), isWithinRangeOfOldTracks));
			if ((newCandidateCenter.has_value()) && isInRange) {
				const auto timestamp = mSurface.at(static_cast<int16_t>(round(newCandidateCenter->y())),
					static_cast<int16_t>(round(newCandidateCenter->x())));
	 ...
	 * Note: kernel weoghts are updated only if the search window changed size or if it intersects the boundaries of the
